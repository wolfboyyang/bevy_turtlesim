...
fn turtle_movement_system(
    mut query: Query<(&Turtle, &mut Transform)>,
    mut move_event_reader: EventReader<MoveEvent>,
    mut lines: ResMut<DebugLines>,
) {
    let (turtle, mut transform) = query.single_mut();

    for event in move_event_reader.iter() {
        let rotation_factor = event.rotation as f32;
        let movement_factor = event.movement as f32;

        let start = transform.translation;

        let angular = rotation_factor * turtle.rotation_speed * TIME_STEP;
        ...
        let movement_distance = movement_factor * turtle.movement_speed * TIME_STEP;
